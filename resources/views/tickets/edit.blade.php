<x-app-layout>
    <x-ticket-form>
        ...
        <form action="{{ route('tickets.set-priority', $ticket) }}" method="POST">
            @method('PATCH')
            @csrf
            <x-ticket-select :disabled="!auth()->user()->can_change_priority" :submittable="auth()->user()->can_change_priority" :selected="$ticket->priority" :name="'priority'" :values="$priorities" />
        </form>
        <div class="flex flex-row justify-right mt-8">
            <form method="POST" action="{{route('tickets.destroy', $ticket)}}">
                @method('DELETE')
                @csrf
                <div x-data="{open: false}">
                    <x-danger-button type="button" @click="open = true">Delete</x-danger-button>
                    <div x-cloak x-show="open" class="relative z-10" aria-labelledby="modal-title" role="dialog" aria-modal="true">
                        ...
                        <div class="fixed inset-0 z-10 w-screen overflow-y-auto">
                            <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                                <div class="relative overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
