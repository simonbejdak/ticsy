@if($table->isPreviousPage() || $table->isNextPage())
    <div class="flex flex-row w-full justify-end my-2 items-center text-gray-300">
        {{-- Double Backwards --}}
        <div
            {{ !$table->isPreviousPage() ? 'disabled' : '' }}
            class="p-1 rounded-md {{ $table->isPreviousPage() ? 'text-slate-800 hover:bg-slate-400 hover:cursor-pointer ' : '' }}"
        ...
        {{-- Forward --}}
        <div
            {{ !$table->isNextPage() ? 'disabled' : '' }}
            class="p-1 rounded-md {{ $table->isNextPage() ? 'text-slate-800 hover:bg-slate-400 hover:cursor-pointer ' : '' }}"
